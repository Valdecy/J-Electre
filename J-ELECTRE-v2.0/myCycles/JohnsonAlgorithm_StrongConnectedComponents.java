...
//	JohnsonAlgorithm_StrongConnectedComponents: Algorithm by D. B. Johnson Finding all the elementary circuits of a directed graph. D. B. Johnson, SIAM Journal on Computing 4, no. 1, 77-84, 1975. http://dx.doi.org/10.1137/0204007
//	JohnsonAlgorithm_StrongConnectedComponents: Copyright @ 26.08.2006 by Frank Meyer - web@normalisiert.de or https://github.com/josch/cycles_johnson_meyer.
//	JohnsonAlgorithm_StrongConnectedComponents: Modified in 01.01.2017 by Valdecy Pereira, Helder Gomes Costa and Livia Dias de Oliveira Nepomuceno.
 ...
 *
 */
public class JohnsonAlgorithm_StrongConnectedComponents {
	...
	/** Helpattribute for finding scc's */
	private Vector<Vector<Integer>> currentSCCs = null;
	 ...
	 */
	public JohnsonAlgorithm_StrongConnectedComponents(int[][] adjList) {
	 ...
	 */
	public JohnsonAlgorithm_SCCResult getAdjacencyList(int node) {
		...
		this.currentSCCs = new Vector<Vector<Integer>>();
					...
					if (adjacencyList != null) {
						for (int j = 0; j < this.adjListOriginal.length; j++) {
							if (adjacencyList[j].size() > 0) {
								return new JohnsonAlgorithm_SCCResult(adjacencyList, j);
							}
						}
					}
				}
	...
	private Vector[] getAdjList(Vector<?> nodes) {
		...
		if (nodes != null) {
			...
			for (int i = 0; i < lowestIdAdjacencyList.length; i++) {
				lowestIdAdjacencyList[i] = new Vector<Object>();
			}
	...
	public static void main(String[] args) {
		...
		int[][] adjList = JohnsonAlgorithm_AdjacencyList.getAdjacencyList(adjMatrix);
		JohnsonAlgorithm_StrongConnectedComponents scc = new JohnsonAlgorithm_StrongConnectedComponents(adjList);
		for (int i = 0; i < adjList.length; i++) {
			System.out.print("i: " + i + "\n");
			JohnsonAlgorithm_SCCResult r = scc.getAdjacencyList(i);
			if (r != null) {
				@SuppressWarnings("rawtypes")
