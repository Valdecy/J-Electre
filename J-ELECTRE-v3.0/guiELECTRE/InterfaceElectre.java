...
import matrixELECTRE.Credibility;
...
public class InterfaceElectre {
	...
	public static void main(String[] args) {
		...
		f.getContentPane().setBackground(SystemColor.controlDkShadow);
		f.setTitle("J-ELECTRE-v3.0 - github.com/Valdecy");
		f.setSize(1076, 644);
...
//		table_1.getColumnModel().getColumn(0).setPreferredWidth(170);
		...
		lblC.setBounds(123, 9, 50, 14);
		panel_1.add(lblC);

		JSpinner spinnerC = new JSpinner();
		spinnerC.setBounds(173, 4, 58, 25);
		...
		buttonGraph.setBounds(385, 4, 63, 23);
		panel_1.add(buttonGraph);

		JButton buttonSave = new JButton("Save");
		buttonSave.setBounds(458, 4, 63, 23);
		panel_1.add(buttonSave);

		JButton btnBuildMatrix = new JButton("Matrix");
		btnBuildMatrix.setBounds(239, 4, 63, 23);
		...
		buttonSolve.setBounds(312, 4, 63, 23);
		...
		buttonSolve.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (electre == 1){
					...
					String[][] dominanceMatrix = new String[q + 1][q + 1];
					String[]   kernelVector    = new String[q];
					for (int i = 0; i < arraySolutionEI.length; i++){
						for (int j = 0; j < arraySolutionEI[0].length; j++){
							if (arraySolutionEI[i][0] == "Dominance Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEI[i + m][n + 1];
									}
								}
							}
							if (arraySolutionEI[i][0] == "Kernel: "){
								for (int m = 0; m < kernelVector.length; m++){
									kernelVector[m] = arraySolutionEI[i][m + 1];
								}
							}
						}
					}
					try {
						jsElectre.writeFile_ei(dominanceMatrix, kernelVector);
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEI.length + 5, 0);
				...
				if (electre == 2){
					...
					double [][] arrayCycles_double = JohnsonAlgorithm_io_int.cyclesElimination(Credibility.getCredibilityMatrixEI_s(getValue, getValueW, getValueP, getValueQ, getValueV),1);
					String [][] arrayCycles_str = new String [arrayCycles_double.length + 1][arrayCycles_double.length + 1];
					for (int i = 0; i < arrayCycles_double.length; i++) {
						for (int j = 0; j < arrayCycles_double[0].length; j++) {
							arrayCycles_str[0][j + 1] = "a" + (j + 1);
							arrayCycles_str[i + 1][0] = "a" + (i + 1);
						}
					}
					for (int i = 0; i < arrayCycles_double.length; i++) {
						for (int j = 0; j < arrayCycles_double[0].length; j++) {
							arrayCycles_str[i + 1][j + 1] = Double.toString(Math.round(arrayCycles_double[i][j] * 10000d)/10000d);
						}
					}
					for (int i = 0; i < arrayCycles_str.length; i++) {
						for (int j = 0; j < arrayCycles_str[0].length; j++) {
							if (arrayCycles_str[i][j] == null){
								arrayCycles_str[i][j] = "";
							}
						}
					}
					...
					for (int i = 0; i < arraySolutionEI_s.length; i++){
						for (int j = 0; j < arraySolutionEI_s[0].length; j++){
							if (arraySolutionEI_s[i][0] == "Credibility Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEI_s[i + m][n + 1];
									}
								}
							}
							if (arraySolutionEI_s[i][0] == "Kernel: "){
								for (int m = 0; m < kernelVector.length; m++){
									kernelVector[m] = arraySolutionEI_s[i][m + 1];
								}
							}
						}
					}
					try {
						jsElectre.writeFile_ei_s(dominanceMatrix, kernelVector, arrayCycles_str);
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEI_s.length + 5, 0);
				...
				if (electre == 3){
					...
					for (int i = 0; i < arraySolutionEI_v.length; i++){
						for (int j = 0; j < arraySolutionEI_v[0].length; j++){
							if (arraySolutionEI_v[i][0] == "Credibility Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEI_v[i + m][n + 1];
									}
								}
							}
							if (arraySolutionEI_v[i][0] == "Kernel: "){
								for (int m = 0; m < kernelVector.length; m++){
									kernelVector[m] = arraySolutionEI_v[i][m + 1];
								}
							}
						}
					}
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEI_v.length + 5, 0);
				...
				if (electre == 4){
					...
					String[][] dominanceMatrix  = new String[q][q];
					String[][] rankMatrix       = new String[q + 1][q + 1];
					String[][] rankMatrix_XY    = new String[q][3];
					String[][] aRanking         = new String[q][2];
					String[][] dRanking         = new String[q][2];
					String[][] mRanking         = new String[q][2];

					for (int i = 0; i < arraySolutionEII.length; i++){
						for (int j = 0; j < arraySolutionEII[0].length; j++){
							if (arraySolutionEII[i][0] == "Dominance Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEII[i + m + 1][n + 2];
									}
								}
							}
							if (arraySolutionEII[i][0] == "Ranking:"){
								for (int m = 0; m < aRanking.length; m++){
									aRanking[m][0] = arraySolutionEII[i + m + 1][1];
									aRanking[m][1] = arraySolutionEII[i + m + 1][2];
									
									dRanking[m][0] = arraySolutionEII[i + m + 1][1];
									dRanking[m][1] = arraySolutionEII[i + m + 1][3];
									
									mRanking[m][0] = arraySolutionEII[i + m + 1][1];
									mRanking[m][1] = arraySolutionEII[i + m + 1][4];
								}
							}
						}
					}
					rankMatrix = Ranking.matrixRanking(dominanceMatrix);
					rankMatrix_XY = Ranking.matrixRanking_XY(rankMatrix);
					try {
						jsElectre.writeFile_eii_eiii_eiv(aRanking, dRanking, mRanking, rankMatrix, rankMatrix_XY);
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEII.length + 5, 0);
				...
				if (electre == 5){
					...
					for (int i = 0; i < arraySolutionEIII.length; i++){
						for (int j = 0; j < arraySolutionEIII[0].length; j++){
							if (arraySolutionEIII[i][0] == "Dominance Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEIII[i + m + 1][n + 2];
									}
								}
							}
							if (arraySolutionEIII[i][0] == "Ranking:"){
								for (int m = 0; m < aRanking.length; m++){
									aRanking[m][0] = arraySolutionEIII[i + m + 1][1];
									aRanking[m][1] = arraySolutionEIII[i + m + 1][2];
									
									dRanking[m][0] = arraySolutionEIII[i + m + 1][1];
									dRanking[m][1] = arraySolutionEIII[i + m + 1][3];
									
									mRanking[m][0] = arraySolutionEIII[i + m + 1][1];
									mRanking[m][1] = arraySolutionEIII[i + m + 1][4];
								}
							}
						}
					}
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEIII.length + 5, 0);
				...
				if (electre == 6){
					...
					for (int i = 0; i < arraySolutionEIV.length; i++){
						for (int j = 0; j < arraySolutionEIV[0].length; j++){
							if (arraySolutionEIV[i][0] == "Dominance Matrix:"){
								for (int m = 0; m < dominanceMatrix.length; m++){
									for (int n = 0; n < dominanceMatrix[0].length; n++){
										dominanceMatrix[m][n] = arraySolutionEIV[i + m + 1][n + 2];
									}
								}
							}
							if (arraySolutionEIV[i][0] == "Ranking:"){
								for (int m = 0; m < aRanking.length; m++){
									aRanking[m][0] = arraySolutionEIV[i + m + 1][1];
									aRanking[m][1] = arraySolutionEIV[i + m + 1][2];
									
									dRanking[m][0] = arraySolutionEIV[i + m + 1][1];
									dRanking[m][1] = arraySolutionEIV[i + m + 1][3];
									
									mRanking[m][0] = arraySolutionEIV[i + m + 1][1];
									mRanking[m][1] = arraySolutionEIV[i + m + 1][4];
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionEIV.length + 5, 0);
				...
				if (electre == 7){
					...
					double [][] projection =  TSVD.getTruncatedSVD(getValue, 2);
					try {
						if (getValue[0].length == 2) {
							jsElectre.writeFile_e_tri(rankMatrix, getValue);
						}else {
							jsElectre.writeFile_e_tri(rankMatrix, projection);
						};
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					...
					table_1.getModel().setValueAt("Livia Nepomuceno", arraySolutionETri.length + 5, 0);
